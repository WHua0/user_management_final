...
It is important to note that before this course, I had a B.S. in Biology and minimal computer-related knowledge aside from gaming. To change my career from a real estate paralegal, I applied and was accepted into NJIT’s Graduate Program for Business and Information Systems. After which, I self-taught my first programming language, Python, about three months before class started. As such, my experiences in this class are from the perspective of a beginner in the field of Computer Sciences.

One of the most important things that I have learned from this course is the use of virtual environments, testing and logging, git and GitHub, and Docker and DockerHub to adhere to The Twelve Factors. By using VENV, one can isolate and maintain the same programming environment or dependencies without being affected by external factors, such as library updates. Through testing and logging, one can verify if the program is working as expected and locate any bugs that appear.  Through git, the codebase revisions can be tracked, and jointly developed by multiple parties on GitHub. Lastly, Docker allows one to transform the codebase into image, and run the image in the container. The container then can be attached to other resources and shared with other parties on DockerHub. However, by using GitHub Actions to connect GitHub to DockerHub, one can streamline the development and operations process. As a such, I have understood the basic steps of DevOps, and its importance.

As I struggled with the final, I learned the importance of following the bouncing ball to code and debug, and to rely on pytest coverage instead of just pytest. While fixing Issues with Create User and Update User, I had thought that the fixed code and tests were working since there were no issues with pytest. However, later when I tried pytest coverage, I found that the tests for the fixes were missing coverage. In other words, either the code or tests were not working as expected. As a result, I had to reopen the issue. Next, I reviewed the logs for error messages and followed the bouncing ball. Only then were the problems identified and addressed properly, otherwise the bugs would have been buried by the code’s increasing complexity and remain a latent issue. 

My only complaint about this course is the vast amount of data and information needed to be understood. Of course, this is from the perspective of an absolute beginner. To keep up, I had to spend three to four times more time than my other classes and search for external sources of information, such as YouTube tutorials. Simply reading about the topic is insufficient, and I needed guidance starting from scratch since I did not have the necessary foundation to understand all course materials. Nevertheless, the struggle was worthwhile, and the reward is essential knowledge in the world of programming.
